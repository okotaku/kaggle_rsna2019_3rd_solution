...
class RSNADataset(Dataset):
    ...
    def __init__(self,
        ...
        if conc_type2 is None:
            self.conc_type2 = conc_type
        else:
            self.conc_type2 = conc_type2
    ...
    def __getitem__(self, idx):
        ...
        if self.conc_type in ["concat3", "concat5"]:
            img_pre = self._get_jpg_img(img_id, "pre", self.conc_type)
            img_post = self._get_jpg_img(img_id, "post", self.conc_type2)
        else:
            img_pre = self._get_jpg_img(img_id, None, self.conc_type)
            img_post = self._get_jpg_img(img_id, None, self.conc_type2)
        target = self.y[idx]
    ...
    def _get_jpg_img(self, img_id, name, conc_type):
        if name is None:
            img_path = os.path.join("../input", conc_type, img_id + ".jpg")
        else:
            img_path = os.path.join("../input", conc_type, img_id + "_" + name + ".jpg")
