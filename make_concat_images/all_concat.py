...
class RSNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_all = cur_idx_row["all_SOPInstanceUID"]

        imgs = []
        for load_id in img_all:
            if load_id == skip_id:
                continue
        ...
        output_path = "../input/concat_all/{}.jpg".format(img_id)
...
LOGGER_PATH = "log.txt"
TRAIN_PATH = os.path.join(DATA_DIR, "train_concat_all.csv")
ID_COLUMNS = "Image"
...
def main():
    with timer('load data'):
        ...
        df = df[df.Image != skip_id].reset_index(drop=True)[2400*batch_size:]
        df["all_SOPInstanceUID"] = df["all_SOPInstanceUID"].map(lambda x: x.split(","))
        df = df[["Image", "all_SOPInstanceUID"]]
        gc.collect()
