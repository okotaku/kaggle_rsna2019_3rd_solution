...
LOGGER_PATH = "log.txt"
TRAIN_PATH = os.path.join(DATA_DIR, "test_concat_st2.csv")
ID_COLUMNS = "Image"
...
use_cols = ["pre1_SOPInstanceUID", "post1_SOPInstanceUID"]
...
class RSNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        load_imgs = cur_idx_row[use_cols].values.reshape(-1)

        self._create(load_imgs, img_id)

        return np.array([0])

    def _create(self, img_pathes, img_id):
        imgs = []
        break_flag = False
        for i, load_id in enumerate(img_pathes):
            if i == 1 and load_id == "UNK" and len(imgs) == 0:
                load_id = img_id
            elif i == 1 and load_id == skip_id and len(imgs) == 0:
                load_id = img_id
        ...
        output_path = "../input/concat_prepost/{}.jpg".format(img_id)
...
def main():
    with timer('load data'):
        ...
        df = df[["Image"]+use_cols].fillna("UNK")
        gc.collect()
