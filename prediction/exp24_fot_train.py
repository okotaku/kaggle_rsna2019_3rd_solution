...
LOGGER_PATH = "log.txt"
TRAIN_PATH = os.path.join(DATA_DIR, "train_concat_double.csv")
ID_COLUMNS = "Image"
...
def main():
    with timer('load data'):
        ...
        df = df[
            ["Image", "pre_SOPInstanceUID", "post_SOPInstanceUID", "prepre_SOPInstanceUID", "postpost_SOPInstanceUID"]]
        ids = df["Image"].values
        gc.collect()
    ...
    with timer('preprocessing'):
        ...
        test_dataset = RSNADatasetTest(df, img_size, IMAGE_PATH, id_colname=ID_COLUMNS,
                                    transforms=test_augmentation, black_crop=False, three_window=True, rescaling=False,
                                    pick_type="post_post", n_tta=N_TTA)
